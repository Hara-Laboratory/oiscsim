...
using Interface.Execute.SubRISC;

namespace Interface.Execute
{
    //この感じなら命令レベルで処理してしまうモデルと
    // サイクル単位でしっかり計算するモデルの両方が作れるね
    public class SubRISCCycleModel : SimulatorModelBase
    {
        ...
        public SubRISCCycleModel(bool delayBranchEnabled)
        {
        ...
        {
            return new SubRISCCycleModel(true);
        }
        ...
        public override bool SetupFromSetupData(ExecuteSetupData setupData)
        {
            ...
            CircuitGroup.CS.RegisterFile.Entries[1].Content = (uint)setupData.MemoryContents[0].WordCapacity * 4 - 4;
