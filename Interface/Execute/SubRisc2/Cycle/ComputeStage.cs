...
namespace Interface.Execute.SubRISC2
{
    public class ComputeStage : SyncModuleBase
    {
        ...
        public ComputeStage()
        {
            ...
            RegisterFile = new SubRISC2.RegisterFile();
            ...
            alu_a_OFace.SetFunc(() =>
            {
                ...
                if (instrIsMemoryOperation)
                    return (uint)(TestBit((uint)instrOperandA, 3) ? 0xFFFFFFF8 : 0u) | (uint)((instrOperandA & 7)); 
        ...
        protected override void UpdateModuleCycle()
        {
            ...
            if (!Stall_IFace)
            {
                ...
                { //Other
                    if (instrIsMemoryOperation && !reg_read1_OFace.Value.Enabled && TestBit(instrOperandB, 2))
                    {
                        var a = PC_IFace >> 1; //Bypass PC
                        AluRes_OFace.Assign(Alu.OpResult_OFace.Value + 0);
                    }
                    else
                    {
