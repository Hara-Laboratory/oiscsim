...
namespace Interface.Assemble.SubRisc2
{
    class InstructionAssembler
    {
        ...
        abstract class InstructionTypeBase
        {
            ...
            protected static bool CheckImmediateOperand(Instruction.OperandElement operand, int rangeMin, int rangeMax, int addressTranslationDiv = 1)
            {
                if (operand.Immediate.Type != ValueBaseType.Immediate)
                    return false;

                int imm = (int)operand.Immediate.GetValue(BytesPerWord, addressTranslationDiv);
            ...
            protected static bool ConvertRegisterName(Instruction.OperandElement operand, int operandIndex, List<AssembleError> errorList)
            {
                ...
                if (!SubRisc2Assembler.RegisterMapping.SearchByName(reginfo.Name, out e))
                {
            ...
            protected virtual bool AssembleJumpBlock(Instruction instr, ref ushort buf, List<AssembleError> errorList)
            {
                ...
                { //Relative address mode
                    ...
                    int relAddr = (int)jumpSpecifier.Immediate.GetValue(BytesPerWord, 1) / 2 - (int)instr.PlacedInfo.Address.From / 2; //分岐先はバイトアドレスで判断する
                    relAddr -= 1;
        ...
        class MemoryreadInstruction : InstructionTypeBase
        {
            ...
            public override bool Assemble(Instruction instr, ushort[] buffer, out int length, out bool needToAlign, List<AssembleError> errorList)
            {
                ...
                int regNumAt1_pc;
                {
                    ...
                    SubRisc2Assembler.RegisterMapping.SearchByName("PC", out r);
                    regNumAt1_pc = r.OperandNumbers[1];
                }
                uint imm = instr.Operands[0].Immediate.GetValue(BytesPerWord, 1); //即値が与えられるのでアドレス変換はしない
                if (instr.Operands[1].RegisterNo == regNumAt1_pc && (int)imm < 0)
                { //PC相対ロードで、PCより後ろの領域を指定している
                    //[Case.1]
                    //0x00: mr -1, $PC, $D; //実行されるときはベースアドレス=0x02
                    //0x02: (padding)
                    //0x04: imm -1;
                    //(ベースアドレス+4)/4 = (0x0 + 1)/4 = 0x04をアクセスしたいので、
                    //「mr -1, $PC, $D;」のオフセットは-1としてコンパイルする
                    //
                    //[Case.2]
                    //0x02: mr -1, $PC, $D; //実行されるときはベースアドレス=0x04
                    //0x04: imm -1; 
                    //(ベースアドレス+0)/4 = (0x04 + 0)/4 = 0x04をアクセスしたいので、
                    //「mr -1, $PC, $D;」のオフセットは0としてコンパイルする

                    imm += 1; //ComputeStageからはPCは1halfword分進んで見える。
                              //PC相対ロードでは、オフセットに次の命令を示す-1を指定するのでそれをもとに戻す。
                    if (instr.Operands[0].PlacedInfo.Address.From / 2 % 2 ==0)
                    { //パディングが挿入される場合はさらに1halfword分オフセットする
                        imm -= 1;
                    } 
                }
        ...
        class MemorywriteInstruction : InstructionTypeBase
        {
            ...
            public override bool Assemble(Instruction instr, ushort[] buffer, out int length, out bool needToAlign, List<AssembleError> errorList)
            {
                ...
                //Process instruction
                {
                    buffer[0] = 0;
                    buffer[0] |= 2 << 14; //Opcode
                    buffer[0] |= (ushort)((instr.Operands[0].Immediate.GetValue(BytesPerWord, 1) & 0xF) << 9); //Offset immediate　//即値が与えられるのでアドレス変換はしない
                    buffer[0] |= (ushort)((instr.Operands[1].RegisterNo) << 4); //Register-B (containing address)
                    buffer[0] |= (ushort)((instr.Operands[2].RegisterNo) << 0); //Register-D (to be written)
        ...
        class ShiftInstruction : InstructionTypeBase
        {
            ...
            protected override bool CheckValidness(Instruction instr, List<AssembleError> errorList)
            {
                ...
                { //Instruction specified amount
                    ...
                    int amount = (int)instr.Operands[2].Immediate.GetValue(BytesPerWord, 1); //即値が与えられるのでアドレス変換はしない
            ...
            public override bool Assemble(Instruction instr, ushort[] buffer, out int length, out bool needToAlign, List<AssembleError> errorList)
            {
                ...
                //Process instruction
                {
                    ...
                    if (isArbitaryAmount)
                        ...
                        buffer[0] |= (ushort)((0x10 | ((instr.Operands[1].Immediate.GetValue(BytesPerWord, 1) / 2) & 0xF)) << 4); //1_(4bit immediate amount) //即値が与えられるのでアドレス変換はしない
                    buffer[0] |= (ushort)((instr.Operands[2].RegisterNo) << 0); //Register-D (to be written)
        ...
        class ImmediateSpaceInstruction : InstructionTypeBase
        {
            ...
            protected override bool CheckValidness(Instruction instr, List<AssembleError> errorList)
            {
                ...
                if (instr.Operands.Length != 1 ||
                    !CheckImmediateOperand(instr.Operands[0], int.MinValue, int.MaxValue, 4)) //アドレスが指定された場合には、バイトアドレス=>ワードアドレス変換を行う
                {
                    errorList.Add(new Interface.Assemble.AssembleError()
                    {
                        Title = "Assemble",
            ...
            public override bool Assemble(Instruction instr, ushort[] buffer, out int length, out bool needToAlign, List<AssembleError> errorList)
            {
                ...
                //Process instruction
                {
                    uint imm = instr.Operands[0].Immediate.GetValue(BytesPerWord, 4); //アドレスが指定された場合には、バイトアドレス=>ワードアドレス変換を行う
